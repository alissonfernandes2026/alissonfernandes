...
# Mapa de históricos por fonte — usados para preencher os resultados
# quando não há itens novos suficientes na execução atual
MAPA_HISTORICOS = {
    "DIARIO": "historico_diario.json",   # dict {idsec-edi: {...}} — gerenciado pelo scraper_diario.py
    "PNCP":   "historico_pncp.json",     # list [{...}]
    "PDCP":   "historico_portal.json",   # list [{...}]
}
...
def _carregar_historico_lista(caminho):
    """Carrega um histórico no formato lista [{...}] de forma segura."""
...
def salvar_por_fonte(novos: list, historico_path: str, config: dict):
    """
    Separa os novos itens por fonte, mescla com o histórico completo de cada
    fonte e mantém as últimas MAX_POR_FONTE entradas nos arquivos de resultado.

    Fluxo por fonte:
      1. Classifica os itens novos da execução atual por fonte
      2. Carrega o histórico completo da fonte (historico_pncp/portal.json)
      3. Marca os itens novos que ainda não existem no histórico
      4. Combina: novos + histórico, truncado em MAX_POR_FONTE
      5. Salva o resultado em resultados_pncp/pdcp/diario.json
    """
    ...
    for fonte, arquivo_padrao in MAPA_ARQUIVOS.items():
        ...
        # Carrega histórico completo da fonte (lista de itens anteriores)
        arq_historico = MAPA_HISTORICOS.get(fonte, "")
        if fonte == "DIARIO":
            # Diário usa resultado anterior (o histórico é dict gerenciado pelo scraper)
            hist_items = _carregar_json(arquivo).get("dispensas", [])
        else:
            hist_items = _carregar_historico_lista(arq_historico)

        ids_existentes = {_id_item(d) for d in hist_items}

        # Enriquece e marca novos itens da execução atual
        ...
        # Combina: itens novos reais + histórico completo, truncado em MAX_POR_FONTE
        realmente_novos = [i for i in novos_fonte if i.get("nova")]
        combinado = realmente_novos + hist_items
